...
class WaymoSegment:
    def __init__(self,segment_path,root_path,train=1):
        self.dataset=tf.data.TFRecordDataset(segment_path, compression_type='')

        self.base_path=root_path
        self.segment_path = segment_path
        self.sequence_name = os.path.basename(segment_path)
    ...
    def _gen_path(self):
        ...
        for folder in ['lidar', 'pose', 'boxes']:
            os.makedirs(self.base_path + '/' + folder, exist_ok=True)
        # for i in range(5):
    ...
    def save_point_cloud(self,frame):
        ...
        np.save(self.base_path + '/' + 'lidar' + '/' + self.save_name + '.npy', top_lidar)
    ...
    def save_point_cloud_label(self,frame,all_label_f,digits=4):
        ...
        box_path = self.base_path + '/boxes/' + self.save_name + '.npy'
        # f = open(self.pcd_label_path + self.save_name+".txt",'w')
        box_list = []
        for label in labels:
            box_dict = {'translation': np.array((label.box.center_x, label.box.center_y, label.box.center_z)),
                        'size': np.array((label.box.length, label.box.width, label.box.height)),
                        'rotation': np.array((0, 0, label.box.heading,)),   # just rot_vec
                        'class': label.type,
                        'nbr_pts': label.num_lidar_points_in_box,
                        'uuid' : label.id,
                        'velocity' : np.array((label.metadata.speed_x, label.metadata.speed_y, 0)),
                        'accelaration' : np.array((label.metadata.accel_x, label.metadata.accel_y, 0)),
                        }
    ...
    def save_poses(self,frame,digits=4):
        ...
        save_poses_path = self.base_path + '/pose/' + self.save_name + '.npy'
        poses = np.reshape(poses, (4, 4))
        np.save(save_poses_path, poses)
        # ret = ""
...
class Waymo():
    def __init__(self,origin_data_path, save_root_path):
    ...
    def convert_all_segment(self):
        ...
        for train_segment in self.train_segment_path:
            ...
            ws = WaymoSegment(train_segment, self.save_root_path)
            ws.save_all_frame()
        f.close()
        ...
        for val_segment in self.val_segment_path:
            ...
            ws = WaymoSegment(val_segment, self.save_root_path, train=0)
            ws.save_all_frame()
        f.close()
...
if __name__ == "__main__":
    ...
    root_path = '/home/vacekpa2/data/waymo/raw/'
    save_path = '/home/vacekpa2/data/waymo/raw/processed/'

    trn_segments = glob.glob('/home/vacekpa2/data/waymo/raw/training/*.tfrecord')
    val_segments = glob.glob('/home/vacekpa2/data/waymo/raw/validation/*.tfrecord')
    tst_segments = glob.glob('/home/vacekpa2/data/waymo/raw/test/*.tfrecord')

    all_segments = trn_segments + val_segments + tst_segments

    seq = int(sys.argv[1])

    record_to_process = all_segments[seq]

    save_path = save_path + '/'.join(record_to_process.split('/')[-2:])
    print(save_path)
    ws = WaymoSegment(all_segments[seq], save_path)
    ws.save_all_frame()
