...
def calculate_yaw(pts1, pts2):
    ...
    for i in range(len(pts1)):
        ...
        yaws.append(yaw_degree)

    return np.stack(yaws)

def calculate_nearest_yaw_error(yaws1, yaws2):
    if (yaws1 > 360).any() or (yaws1 < 360).any() or (yaws2 > 360).any() or (yaws2 < 360).any():
        raise ValueError('Degrees over 360')

    # a = np.array((10, 25))
    # b = np.array((-240, -180))
    res = np.zeros(yaws1.shape[0])
    delta = np.abs(yaws1 - yaws2) % 360

    mask1 = delta > 180
    mask2 = delta <= 180

    res[mask1] = 360 - delta[mask1]
    res[mask2] = delta[mask2]
